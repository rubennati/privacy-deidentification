...
| `text_geometry` | ✅ OCR L10 (field on `text_result`) | per-page line boxes mapping technical raw spans (persisted `canonical_*` compatibility names) to page-local bounds, with status/coverage; no raw line text | no raw text (offsets + bounds only) | additive optional versioned field on `text_result` |
...
- **Text artifacts** contain extracted text and therefore may contain PII. `reading_text` is subject
  to the same boundary as technical raw, readable, layout, block, and PII-input text. They remain under the
