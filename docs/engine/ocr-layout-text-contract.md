...
- **`pii_input_text`, `readable_text`, `layout_text_result`, `layout_blocks`, `text_geometry`,
  `structured_content`, and `text_lineage_map` are additive** — new optional fields/artifacts.
...
- **Delivered (L11):** `structured_content_version = "1"` and optional `structured_content` with
  ...
  benchmark loaders ignore the payload. Canonical text and active PII input are unchanged.
...
- Any implementation must prove:
  ...
  - `readable_text`, `layout_text_result`, `pii_input_text`, `layout_blocks`, `structured_content`,
    and `text_lineage_map` are optional;
